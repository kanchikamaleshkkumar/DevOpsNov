...
LOG=/tmp/stack.log
ID=$(id -u)
MOD_JK_URL=$( curl -s https://tomcat.apache.org/download-connectors.cgi | grep nofollow | grep tar | cut -d '"' -f2)
MOD_JK_TAR_FILE=$(echo $MOD_JK_URL | awk -F / '{print $NF}') 
...
STUDENT_WAR_URL=https://github.com/kanchikamaleshkkumar/DevOpsNov/raw/master/APPSTACK/student.war
MYSQL_DRIVER_URL=https://github.com/kanchikamaleshkkumar/DevOpsNov/raw/master/APPSTACK/mysql-connector-java-5.1.40.jar
...
VALIDATE $? "Enabiling the webserver"
...
grant all privileges on studentapp.* to 'student'@'localhost' identified by 'student@1';" > /tmp/studentapp.sql

VALIDATE $? "Creating studentapp.sql"
